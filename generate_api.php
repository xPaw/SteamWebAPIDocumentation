...
if( file_exists( __DIR__ . '/api_web_bundle_calls.json' ) )
{
	$WebBundleCalls = json_decode( file_get_contents( __DIR__ . '/api_web_bundle_calls.json' ), true, 512, JSON_THROW_ON_ERROR );
}
else
{
	$WebBundleCalls = [];
...
MergeLists( $FinalList, $WebBundleCalls, 'undocumented' );
					...
					foreach( $Method[ 'parameters' ] as $Parameter )
					{
						...
						foreach( $FinalList[ $InterfaceName ][ $MethodName ][ 'parameters' ] as $ParameterId => $CurrentParameter )
						{
							if( $Parameter[ 'name' ] === $CurrentParameter[ 'name' ] )
							{
								...
								if( isset( $Parameter[ 'enum_values' ] ) )
								{
									$FinalList[ $InterfaceName ][ $MethodName ][ 'parameters' ][ $ParameterId ][ 'enum_values' ] = $Parameter[ 'enum_values' ];
								}
